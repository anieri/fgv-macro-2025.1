...
Para garantir a robustez e a replicabilidade da análise, este trabalho utiliza um arcabouço de engenharia de dados e modelagem econométrica aplicado sobre séries históricas da Coreia do Sul. A integração de dados de diferentes frequências e a derivação de variáveis macroeconômicas seguiram os procedimentos descritos abaixo.

### 2.1. Fontes e Integração de Dados
Os dados foram coletados via APIs oficiais de quatro provedores principais:
1. **FRED (Federal Reserve Economic Data):** Séries de alta frequência para Inflação (YoY), Taxas de Juros, Câmbio Nominal e PIB Real Trimestral.
2. **Banco Mundial (WDI):** Indicadores estruturais anuais, incluindo Valor Adicionado por setor (Agropecuária, Indústria e Serviços) e gastos em saúde.
3. **Banco da Coreia (ECOS/BOK):** Dados históricos de endividamento público e externo em moeda local e USD.
4. **OCDE:** Índices de confiança (CCI e BCI) e benchmarks de crescimento e inflação.

As séries foram harmonizadas em um DataFrame mensal. Para preservar a integridade das fontes, não foi utilizada interpolação em dados trimestrais ou anuais; em vez disso, as análises e gráficos tratam os períodos faltantes de forma a refletir apenas os pontos de dados efetivamente reportados.

### 2.2. Cálculo do Hiato do Produto (Output Gap)
O hiato do produto ($y - y_e$) foi calculado para mensurar o grau de utilização da capacidade produtiva e fundamentar o modelo IS-PC-MR.
- **Método:** Foi aplicado o **Filtro Hodrick-Prescott (HP)** sobre o logaritmo natural do PIB Real Trimestral.
- **Parâmetro de Suavização:** Utilizou-se $\lambda = 1600$, valor padrão recomendado pela literatura para dados de frequência trimestral.
- **Fórmula:** O hiato percentual é definido como $Gap = (\exp(cycle) - 1) \times 100$, onde $cycle$ representa a diferença entre o log do PIB observado e sua tendência estimada pelo filtro.

### 2.3. Taxa de Câmbio Real ($q$)
Para analisar a competitividade externa no modelo AD-BT-ERU, calculamos a taxa de câmbio real bilateral com os Estados Unidos.
- **Fórmula:** $q = E \times \frac{P_{US}}{P_{KOR}}$, onde $E$ é a taxa de câmbio nominal (Won/USD), $P_{US}$ é o índice de preços ao consumidor (CPI) dos EUA e $P_{KOR}$ é o CPI da Coreia do Sul.
- **Normalização:** A série foi indexada para $q = 1$ no início do período de análise para facilitar a identificação de movimentos de depreciação ($q \uparrow$) e apreciação ($q \downarrow$) real.

### 2.4. Taxa de Juros Real ($r$)
A postura da política monetária foi avaliada através da taxa de juros real ex-post.
- **Fórmula:** $r = i - \pi$, onde $i$ representa a taxa básica de juros (Policy Rate) e $\pi$ é a inflação acumulada em 12 meses (CPI YoY).

### 2.5. Indicadores Estruturais e Financeiros
- **Grau de Abertura:** Calculado como a soma das Exportações e Importações dividida pelo PIB nominal.
- **Relação Dívida/PIB:** Para as séries do BOK, a dívida nominal (em bilhões de Won) foi dividida pelo PIB Nominal Anualizado (estimado como o PIB real trimestral multiplicado por 4 e inflacionado pelo índice de preços correspondente).
- **Modelagem Teórica:** Os diagramas IS-PC-MR e AD-BT-ERU foram parametrizados para refletir os deslocamentos qualitativos observados nos dados durante cada crise, servindo como uma ponte entre a evidência empírica e a teoria de Carlin & Soskice (2015).

---

